    ...
    return (
        <>
            <div className="block text-center">
                ...
                <Link href={linkHref} className={`group flex items-center text-black pt-3 text-lg mx-auto md:text-2xl md:ml-[10px] justify-center md:justify-start ${poppins.className}`}>
                    <h3 className="pr-2">
                        {title}
                    </h3>
                    <FaArrowRight className="text-sm md:text-xl transition-transform duration-300 group-hover:translate-x-1" />
                </Link>
                <p className={`text-md md:text-md text-black text-justify pt-2.5 mx-auto w-[80%] md:mx-0 mb-[30px] md:ml-[10px] md:w-[80%] leading-6 ${poppins.className}`}>{description}</p>
            </div>
        </>
    )
}